...
def ThetaOfT(time, initVal=0, var=1):
    """Creates the random walk theta for the time vector with specified variance"""
    stepSize = time[1] - time[0]
    N = len(time)
    return np.cumsum(np.insert(np.random.normal(scale=np.sqrt(var*stepSize), size=(1, N-1)), initVal, 0))
#gain_jitter_list = np.array([0,0.1,0.3,0.5,1,2,3,4,5,7,10,15,20])
gain_jitter_list = np.array([0,1,7,20])
nb_jitters = len(gain_jitter_list)
output = np.zeros((nb_epochs,nt,nb_tests,nb_jitters))



for jit_i in range(nb_jitters):
    gain_jitter = gain_jitter_list[jit_i]
    for test_i in range(nb_tests):
        ...
        input_res_train = np.zeros((N_in,nt)) #Input to the reservoir
        ...
        for ep_i in range(nb_epochs):
            jitter = np.zeros((N_in,n_step_stim))
            for inp_cell in range(N_in):
                for t in range(n_step_stim):
                    jitter[inp_cell,t] = jitter[inp_cell,t-1] + np.random.normal(0,np.sqrt(gain_jitter*dt))
                input_res_train[inp_cell,t_stim:t_stim+n_step_stim] =  A*(np.sin(2*np.pi*osc_periods[inp_cell]*(np.linspace(0,len_stim,n_step_stim)+phase[inp_cell])+jitter[inp_cell,:]) + 1)/2 
            #Simulation variables
            print("Running jitter {}/{}, test {}/{}, epoch {} of {}.".format(jit_i+1,nb_jitters,test_i+1,nb_tests,ep_i+1,nb_epochs))
            r = np.zeros(net.NE)
            hr = np.zeros(net.NE)                       
            ...
            for t in range(nt):
                ...
                Inp_current = np.dot(net.w_res,inp_net[:,t])
...
with open(output_path+'output_panel_d.p', 'wb') as f:                
    pickle.dump(results,f)
    
with open(output_path+'output_panel_d.p', 'rb') as f:                
    results = pickle.load(f)
    
output = results['outputs']
targets = results['targets']
    
corr_cond = np.zeros((nb_jitters,nb_tests))
for jit_i in range(nb_jitters):
    for test_i in range(nb_tests):
        corr_cond[jit_i,test_i] = stats.pearsonr(output[-1,t_stim:,test_i,jit_i],targets[test_i,t_stim:])[0]


avg_corr = np.mean(corr_cond,axis=1)
sem_corr = stats.sem(corr_cond,axis=1)
plt.figure(figsize=(6,3))
plt.errorbar(gain_jitter_list,avg_corr,yerr=sem_corr,color='k')
plt.xlabel('Sigma')
